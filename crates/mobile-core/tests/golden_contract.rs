...
//!              cross-language source of truth shared with the Swift and Kotlin
//!              boundary tests. Here we assert the Rust core honours the fields
//!              that fixture declares for the `rust_core.draft` / `append_event`.
//!
...
fn core_draft_honours_the_fixture_contract() {
    ...
    assert_eq!(draft.workflow_id.as_str(), expected["workflow_id"].as_str().unwrap());
    assert_eq!(draft.draft_id.as_str(), expected["draft_id"].as_str().unwrap());
    assert_eq!(draft.latent_need, expected["latent_need"].as_str().unwrap());
    assert_eq!(draft.contradiction, expected["contradiction"].as_str().unwrap());
    assert_eq!(draft.consent_state.as_str(), expected["consent_state"].as_str().unwrap());
...
fn core_append_event_honours_the_fixture_contract() {
    ...
    assert_eq!(event.event_type.as_str(), expected["event_type"].as_str().unwrap());
    assert_eq!(event.sync_state.as_str(), expected["sync_state"].as_str().unwrap());
}
